...
% debug
% set final condition constraint, test
%mpc_c.eq_con_init_state = [mpc_c.eq_con_init_state; mpc_v.x_ref_arr(:,N+1)-mpc_v.x_arr(:,N)]; 
...
for k = 1:N
    ...
    for leg_k = 1:2
        ...
        % constrant leg on ground, z = current terrain height 
        mpc_c.eq_con_foot_contact_ground(2*(k-1)+leg_k) =...
            contact_mat_t(leg_k)*fp_g_t(3*(leg_k-1)+3) - mpc_v.fp_ref_arr(3*leg_k, k); % * 2 legs
