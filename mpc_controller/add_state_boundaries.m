...
% state
ub_state = [3*pi*ones(3,1);100*ones(2,1);100*ctr_p.max_jump_z;...
            5*pi*ones(3,1);50*ones(2,1);10*ctr_p.max_lift_vel_z];
lb_state = [-3*pi*ones(3,1);-100*ones(2,1);-100*ctr_p.min_dump_z;...
            -5*pi*ones(3,1);-5*ones(3,1)];
...
% foot pos
ub_fp = repmat([0.2;  0.8; inf],2,1); % for 2 legs
lb_fp = repmat([-0.2; -0.8; -inf],2,1);
